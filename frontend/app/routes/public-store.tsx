...
export default function PublicStore() {
  ...
  if (error) {
    return (
      <main className="flex min-h-screen items-center justify-center p-8 text-center dark:bg-stone-800">
  ...
  if (store === undefined) {
    return (
      <main className="flex min-h-screen items-center justify-center dark:bg-stone-800">
  ...
  if (store === null) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-3 p-8 text-center dark:bg-stone-800">
  ...
  return (
    <main className="min-h-screen bg-amber-50/40 dark:bg-stone-800">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {store.snsLinks.map((link) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-full border border-amber-200 bg-white px-3 py-1 text-xs font-semibold text-amber-800 transition-colors hover:bg-amber-50 dark:border-amber-800 dark:bg-stone-700 dark:text-amber-400 dark:hover:bg-stone-600"
              >
          ...
          <div className="mt-10">
            ...
            <ul className="space-y-3">
              {store.menuItems.map((item) => {
                ...
                <li
                  key={item.id}
                  className="flex gap-4 rounded-lg border border-amber-100 bg-white p-4 dark:border-stone-600 dark:bg-stone-700"
                >
                  {itemPhoto && (
                    <img
