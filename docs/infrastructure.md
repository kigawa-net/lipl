...
4. **harbor-registry pull secret（PRごとに動的生成されるdev namespace向け）**: `kigawa-system/secret-provider` の既存CronJobは静的な `TARGET_NAMESPACES` リストのため、`platform-lipl-dev-pr-<PR番号>` のような動的namespaceには対応できない。CronJob側を「`platform-lipl-dev-pr-*` にマッチする既存namespaceを動的に列挙してsyncする」ロジックに変更する必要がある（別PRで対応予定）。stg（`platform-lipl-stg`、静的）は [kigawa-net-k8s#195](https://github.com/kigawa-net/kigawa-net-k8s/pull/195) の静的リスト追加で解決済み
